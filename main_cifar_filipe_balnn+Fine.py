...
def evaluate(dataloader, encoder, classifier, args, noisy_label, clean_label, i, stat_logs):
    ...
    with torch.no_grad():
        ...
        if i>=args.warmup:
            ...
            elif args.operation == "union_fine_g3_g4":
                temp_id = [idx for idx in clean_id.cpu().numpy().tolist() if (idx in otsu_split['maybe_clean_ids']) or (idx in otsu_split['clean_ids'])  ]

                for idx in teacher_idx_1:
                    if (idx.item() in otsu_split['noisy_ids']) or (idx.item() in otsu_split['maybe_noisy_ids']):
                        temp_id.append(idx.item())
                clean_id = torch.tensor(temp_id).cuda()

            elif args.operation == "union_fine_g1":
                temp_id = [idx for idx in clean_id.cpu().numpy().tolist() if (idx in otsu_split['maybe_clean_ids']) or (idx in otsu_split['maybe_noisy_ids']) or (idx in otsu_split['noisy_ids'])  ]

                for idx in teacher_idx_1:
                    if (idx.item() in otsu_split['clean_ids']):
                        temp_id.append(idx.item())
                clean_id = torch.tensor(temp_id).cuda()

            elif args.operation == "union_fine_g1_g2_g3":
                temp_id = [idx for idx in clean_id.cpu().numpy().tolist() if (idx in otsu_split['noisy_ids'])  ]

                for idx in teacher_idx_1:
                    if (idx.item() in otsu_split['clean_ids']) or (idx.item() in otsu_split['maybe_clean_ids']) or (idx.item() in otsu_split['maybe_noisy_ids']):
                        temp_id.append(idx.item())
                clean_id = torch.tensor(temp_id).cuda()

            
            elif args.operation == "union_fine_g2_g3_g4":
                temp_id = [idx for idx in clean_id.cpu().numpy().tolist() if (idx in otsu_split['clean_ids'])  ]

                for idx in teacher_idx_1:
                    if (idx.item() in otsu_split['noisy_ids']) or (idx.item() in otsu_split['maybe_clean_ids']) or (idx.item() in otsu_split['maybe_noisy_ids']):
                        temp_id.append(idx.item())
                clean_id = torch.tensor(temp_id).cuda()

            #adaptive selection based on selection rate
            #elif args.operation == "sr":
            elif args.operation in ["sr", "srbm"]:

                temp_id_ssr_g1g2 = [idx for idx in clean_id.cpu().numpy().tolist() if (idx in otsu_split['clean_ids']) or (idx in otsu_split['maybe_clean_ids'])  ]
                temp_id_ssr_g3g4 = [idx for idx in clean_id.cpu().numpy().tolist() if (idx in otsu_split['maybe_noisy_ids']) or (idx in otsu_split['noisy_ids'])  ]
                
                temp_id_g1g2 = []
                temp_id_g3g4 = []
                for idx in teacher_idx_1:
                    if (idx.item() in otsu_split['clean_ids']) or (idx.item() in otsu_split['maybe_clean_ids']):
                        temp_id_g1g2.append(idx.item())
                    elif (idx.item() in otsu_split['maybe_noisy_ids']) or (idx.item() in otsu_split['noisy_ids']):
                        temp_id_g3g4.append(idx.item())
                fine_g1g2_sr = len(temp_id_g1g2)/(len(otsu_split['clean_ids'])+len(otsu_split['maybe_clean_ids']))
                ssr_g1g2_sr = len(temp_id_ssr_g1g2)/(len(otsu_split['clean_ids'])+len(otsu_split['maybe_clean_ids']))
                fine_g3g4_sr = len(temp_id_g3g4)/(len(otsu_split['maybe_noisy_ids'])+len(otsu_split['noisy_ids']))
                ssr_g3g4_sr = len(temp_id_ssr_g3g4)/(len(otsu_split['maybe_noisy_ids'])+len(otsu_split['noisy_ids']))
                
                #g1g2: fine
                if  (fine_g1g2_sr > ssr_g1g2_sr) :
                    choice_sr_g1g2 = 1
                ...
                #g3g4: fine
                # if (fine_g3g4_sr>ssr_g3g4_sr) and (fine_g3g4_sr <= 0.3):
                #     #choice_sr_g3g4 = 1
                # #g3g4: ssr
                # else:
                    #choice_sr_g3g4 = 0
                #if both models are above threshold, rely on FINE
                if (args.operation == "srbm") and (fine_g3g4_sr>0.3) and (ssr_g3g4_sr > 0.3):
                    choice_sr_g3g4 = 1
                else:
                    choice_sr_g3g4 = 0
                ...
                #fine - fine
                #if  (fine_g1g2_sr > ssr_g1g2_sr)  and (fine_g3g4_sr>ssr_g3g4_sr):
                if (choice_sr_g1g2==1) and (choice_sr_g3g4==1):
                    # clean_id = teacher_idx_1
                    clean_id = torch.tensor(teacher_idx_1).cuda()
                    # choice_sr_g1g2 = 1
                    # choice_sr_g3g4 = 1
                #fine - ssr
                #elif (fine_g1g2_sr > ssr_g1g2_sr)  and (ssr_g3g4_sr>fine_g3g4_sr):
                elif (choice_sr_g1g2==1) and (choice_sr_g3g4==0):
                    temp_id = [idx for idx in clean_id.cpu().numpy().tolist() if (idx in otsu_split['maybe_noisy_ids']) or (idx in otsu_split['noisy_ids'])  ]
                    for idx in teacher_idx_1:
                        if (idx.item() in otsu_split['clean_ids']) or (idx.item() in otsu_split['maybe_clean_ids']):
                            temp_id.append(idx.item())
                    ...
                    # choice_sr_g3g4 = 0
                #ssr - fine
                #elif (ssr_g1g2_sr > fine_g1g2_sr  )  and (fine_g3g4_sr > ssr_g3g4_sr):
                elif (choice_sr_g1g2==0) and (choice_sr_g3g4==1):
                    temp_id = [idx for idx in clean_id.cpu().numpy().tolist() if (idx in otsu_split['maybe_clean_ids']) or (idx in otsu_split['clean_ids'])  ]
                    for idx in teacher_idx_1:
                        if (idx.item() in otsu_split['noisy_ids']) or (idx.item() in otsu_split['maybe_noisy_ids']):
                            temp_id.append(idx.item())
                    ...
                    # choice_sr_g3g4 = 1
                #ssr - ssr
                #elif (ssr_g1g2_sr > fine_g1g2_sr  )  and (ssr_g3g4_sr >fine_g3g4_sr ):
                elif (choice_sr_g1g2==0) and (choice_sr_g3g4==0):
                    #already calculated clean_id
                    # choice_sr_g1g2 = 0
                    # choice_sr_g3g4 = 0
                    pass
