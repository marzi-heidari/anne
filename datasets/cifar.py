...
class CIFAR100_DM(torchvision.datasets.CIFAR100):
    def __init__(self, root, ood_noise, ind_noise, mode, train=True, transform=None, target_transform=None, download=False, pred=[], probability=[], idx_remove=[]):
        super(CIFAR100_DM, self).__init__(root, train=train, transform=transform, target_transform=target_transform, download=download)
        #CIFAR100red
        self.train = train
        ...
        n_class = 100
        self.clean_label = np.array(self.targets)
        ...
        if train and self.ood_noise > 0:
            ...
            self.clean_label[self.ids_ood] = 10000
            del imagenet32
        ...
        if self.mode in  ['all','test']:
            pass  #do nothing, it already has all data and targets
        else:
            if self.mode == "labeled":
                pred_idx = pred.nonzero()[0]   
                pred_idx = [i for i in pred_idx if i not in idx_remove]   #remove samples if required
                self.probability = [probability[i] for i in pred_idx]   
            elif self.mode == "unlabeled":
                pred_idx = (1-pred).nonzero()[0]  
                pred_idx = [i for i in pred_idx if i not in idx_remove]   #remove samples if required
            self.data = self.data[pred_idx]
            self.targets = [self.targets[i] for i in pred_idx]                          
