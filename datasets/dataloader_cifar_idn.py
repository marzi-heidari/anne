...
# from utils.tools import get_instance_noisy_label
        ...
        if self.mode=='test':
            ...
            if os.path.exists(noise_file):
                ...
                if noise_mode == 'instance':
                    ...
                    noise_label = tools.get_instance_noisy_label(self.r, dataset, train_label, num_classes = num_classes_, feature_size = 32*32*3, norm_std=0.1, seed=123)
...
class cifar_dataloader():  
    ...
    def run(self,mode,pred=[],prob=[], args=None):
        if mode=='warmup':
            ...
            labeled_dataset = cifar_dataset(dataset=self.dataset, noise_mode=self.noise_mode, r=self.r, root_dir=self.root_dir, transform=transform_constrastive, mode="labeled", noise_file=self.noise_file, pred=pred, probability=prob,log=self.log)              
            labeled_trainloader = DataLoader(
                dataset=labeled_dataset, 
