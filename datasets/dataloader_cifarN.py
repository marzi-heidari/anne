...
class cifar_dataset(Dataset):
    def __init__(self, dataset, root_dir, transform,  noise_type='',
                 dataset_mode='train',  noise_file=None, print_show=False):
        ...
        if self.mode == 'test':
            ...
            if noise_type != 'clean':
                ...
                self.noise_or_not = np.transpose(self.train_noisy_labels) != np.transpose(self.clean_label)
            ...
            # #     self.cifar_label = noise_label
            # else:
            #     raise ValueError(f'Dataset noise file not found!')
                # # inject noise
    ...
    def load_label(self):
        ...
        if isinstance(noise_label, dict):
            if "clean_label" in noise_label.keys():
                ...
                assert torch.sum(torch.tensor(self.clean_label) - clean_label) == 0
